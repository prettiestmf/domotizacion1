...
for($i = 0;$i < count($consulta);$i++) {
    ...
    $consulta[$i]['acciones'] = '<div class="text-center">
            <button class="btn btn-primary btn-sm" title="Editar" onclick="editarUsuario('.$consulta[$i]['usuario_id'].')">Editar</i></button>
            <button class="btn btn-danger btn-sm" title="Eliminar" onclick="eliminarUsuario('.$consulta[$i]['usuario_id'].')">Eliminar</i></button>
                               </div> ';
